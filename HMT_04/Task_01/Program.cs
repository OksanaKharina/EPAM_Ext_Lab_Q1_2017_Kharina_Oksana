    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < str.Length; i++)
            {
               ...
               else if (char.IsLetter(length[i]) || char.IsDigit(length[i])) // будем считать, что слово может состоять из чисел или содержать число//todo pn неа :) давай учитывать всё, что может ввести пользователь (скрин в корне проекта)
                {
                    symbol += 1;
                }
            }
